...
-- Step 2: Creating tables (with Primary Key and Foreign Key)
...
-- i. Patients born after 2000

SELECT * 
FROM patients
WHERE dob > '2000-01-01';

-- ii. Prescription for patient_id = 5

SELECT *
FROM prescriptions
WHERE patient_id = 5;

-- iii. Appointments per month
...
-- iv. Medications with "pain" or "infection"
...
-- v. Doctors with hospital name

SELECT d.name, d.specialty, h.name AS hospital
FROM doctors d
JOIN hospitals h ON d.hospital_id = h.hospital_id;

-- vi. Patients with August 2025 appointments
...
-- vii. Rooms in Neurology department
...
-- viii. Doctors per hospital

SELECT hospital_id, COUNT(*) AS doctor_count
FROM doctors
GROUP BY hospital_id;

-- ix. Patients with >3 appointments
...
-- x. Appointments with patient and doctor names
...
--------------------------------------------------------------------
-- QUERIES PART 2 (5 MARKS EACH)
---------------------------------------------------------------------
-- 1. Emergency + Age group (CASE)
...
-- 3. Patients with NO prescriptions
...
-- 4. Patients visting multiple hospitals
...
-- 5. Latest appointment per patient
...
-- 6. 3rd most prescribed medication
...
-- 7. Hospitals with lowest doctor count
...
-- 8. Second largest room capacity per hospital
SELECT *
FROM (
    SELECT d.hospital_id,
           r.capacity,
           DENSE_RANK() OVER(PARTITION BY d.hospital_id ORDER BY r.capacity DESC) AS rnk
    FROM rooms r
    JOIN departments d ON r.department_id = d.department_id
) t
WHERE rnk = 2;
...
--------------------------------------------------------------------
-- BONUS IDEAS (EXTRA 20 MARKS)
--------------------------------------------------------------------

-- Top Hospital by cardiologists 

SELECT h.name, COUNT(*) AS cardiologists
FROM doctors d
JOIN hospitals h ON d.hospital_id = h.hospital_id
...
SELECT d.hospital_id, AVG(r.capacity)
FROM rooms r
JOIN departments d ON r.department_id = d.department_id

-- Most common appointment reason
